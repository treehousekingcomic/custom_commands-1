...
class Management(commands.Cog):
    ...
    async def is_premium(self, guild):
        res = await self.bot.db.fetchrow(
            "SELECT * FROM keys WHERE guild = $1", guild.id
        )
        if res:
            return True
        else:
    ...
    async def backup(self, ctx):
        ...
        m = await ctx.send(
            f"{self.bot.get_emoji(700698346784030810)} Data backup in progress..."
        )
        await self.bot.db.execute(
            "INSERT INTO backups(guild, userid, key, aud) VALUES ($1, $2, $3, $4)",
            ctx.guild.id,
            ctx.author.id,
            backup_code,
            aud,
        ...
        await m.edit(
            ...
            + f" Data backup successfull. Code `{backup_code}`. Use this code to restore commands anytime.\n\n"
    ...
    async def list(self, ctx):
        backups = await self.bot.db.fetch(
            "SELECT * FROM backups WHERE guild = $1", ctx.guild.id
        )
        ...
        pages = menus.MenuPages(
            source=MySource(lines, title=f"Backups"), clear_reactions_after=True
        )
        await pages.start(ctx)

    @backup.command()
    async def info(self, ctx, code):
        backups = await self.bot.db.fetchrow(
            "SELECT * FROM backups WHERE guild = $1 and key = $2", ctx.guild.id, code
        )
        if not backups:
    ...
    async def restore(self, ctx, code, password=None):
        ...
        if data["public"] == "yes":
            if data["password"] and data["owner"] != ctx.author.id:
                if not password:
                    ...
                    try:
                        password_ = await self.bot.wait_for(
        ...
        if access1:
            ...
            try:
                ...
                def check(reaction, user):
                    return str(reaction.emoji) in [emoji1, emoji2] and (
                        user.id == ctx.author.id and reaction.message.id == delete.id
                    )
            ...
            # Replace or rename
            if not can_delete:
                replace = await ctx.send(
                    f"If two command has same name do you want to replace them? React with {emoji1} if you want to replace. If you react with {emoji2} commands with same name will be renamed."
                )
                ...
                try:
                    ...
                    def check(reaction, user):
                        return str(reaction.emoji) in [emoji1, emoji2] and (
                            ...
                            and reaction.message.id == replace.id
                        )
            ...
            if can_delete:
                await self.bot.db.execute(
                    "DELETE FROM commands WHERE guild = $1", ctx.guild.id
                )
                await self.bot.db.execute(
                    "DELETE FROM variables WHERE guild = $1", ctx.guild.id
                )
                ...
                for _id, info in cmds.items():
                    ...
                    if xc:
                        ...
                        if not can_replace:
                            ...
                            await self.bot.db.execute(
                                "DELETE FROM commands WHERE name = $1 and guild = $2",
                                name,
                                ctx.guild.id,
                            )
                    else:
            ...
            if variables:
                for var_id, vdata in variables.items():
                    ...
                    if not ctx.guild.get_member(userid):
                        userid = ctx.author.id
                    if (
                        await only_alias_exists(self.bot, name, ctx.guild)
                        and not can_delete
                    ):
                        name = name + str(random.randint(1, 9))
                        await self.bot.db.execute(
    ...
    async def changepassword(self, ctx, code, new_password):
        ...
        if data["owner"] == ctx.author.id:
            ...
            if data["public"] == "yes" and data["guild"] == ctx.guild.id:
                return await ctx.send(
                    "You dont have permissions to change password of this backup."
                )
            else:
                return await ctx.send(f"No backup found with code `{code}`.")
    ...
    async def makepublic(self, ctx, code):
        ...
        if data["owner"] == ctx.author.id:
            ...
            await self.bot.db.execute(
                "UPDATE backups SET aud = $1 WHERE key = $2", True, code
            )
            ...
            if data["public"] == "yes" and data["guild"] == ctx.guild.id:
                return await ctx.send(
                    "You dont have permissions to make this backup public."
                )
            else:
                return await ctx.send(f"No backup found with code `{code}`.")
    ...
    async def makeprivate(self, ctx, code):
        ...
        if data["owner"] == ctx.author.id:
            ...
            await self.bot.db.execute(
                "UPDATE backups SET aud = $1 WHERE key = $2", False, code
            )
