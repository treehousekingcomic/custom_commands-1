...
class Backup(commands.Cog):
    ...
    async def backup(self, ctx):
        ...
        for guild in self.bot.guilds:
            ...
            settings = await self.bot.db.fetchrow("SELECT * FROM guild_data WHERE guild = $1", guild.id)
            settings_data = {
            ...
            commands_ = await self.bot.db.fetch("SELECT * FROM commands WHERE guild = $1", guild.id)
            variables = await self.bot.db.fetch("SELECT * FROM variables WHERE guild = $1", guild.id)
    ...
    async def insert_command(self, userid, guild, name, type, help, approved, data, aliases):
        await self.bot.db.execute("INSERT INTO commands(userid, guild, name, type, help, approved) VALUES ($1, $2, $3, $4, $5, $6)", userid, guild, name, type, help, approved)
        command = await self.bot.db.fetchrow("SELECT * FROM commands WHERE name = $1 and guild = $2", name, guild)

        command_id = command['id']

        if type == 'embed':
            await self.bot.db.execute("INSERT INTO embed(command_id, title, description, thumbnail, image) VALUES ($1, $2, $3, $4, $5)", command_id, data['title'], data['description'], data['thumbnail'], data['image'])

        if type == 'text':
            await self.bot.db.execute("INSERT INTO text (command_id, content) VALUES ($1, $2)", command_id, data['content'])

        if type == 'role':
            await self.bot.db.execute("INSERT INTO role (command_id, role, action) VALUES ($1, $2, $3)", command_id, data['role'], data['action'])

        if type == 'mrl':
            await self.bot.db.execute("INSERT INTO multirole (command_id, gives, removes) VALUES ($1, $2, $3)", command_id, data['gives'], data['removes'])

        for alias_id, data in aliases.items():
            await self.bot.db.execute("INSERT INTO aliases(name, cmd_id, user_id, guild) VALUES ($1, $2, $3, $4)", data['name'], command_id, data['user_id'], guild)
    ...
    async def restore(self, ctx):
        ...
        for guild, datas in data.items():
            ...
            try:
                await self.bot.db.execute("INSERT INTO guild_data(guild, prefix, noprefix, cprefix) VALUES ($1, $2, $3, $4)", guild_id, settings['prefix'], settings['noprefix'], settings['cprefix'])
